      ...
      <section className="relative min-h-[80vh] flex items-center overflow-hidden">
        ...
        <div className="container-app relative py-20">
          <div className="max-w-2xl">
            <p className="text-ember-400 text-sm tracking-widest uppercase mb-4">{DEMO_RESTAURANT.city}</p>
            <h1 className="heading-xl mb-6">
              Bold flavors, <span className="gradient-text">crafted with fire.</span>
            </h1>
            <p className="text-lg text-white/70 mb-8 max-w-xl">{DEMO_RESTAURANT.description}</p>
            <div className="flex flex-wrap gap-3">
              <Link to="/menu" className="px-6 py-4 rounded-2xl bg-flame-gradient text-white font-semibold flex items-center gap-2 shadow-glow">
                <Flame className="w-5 h-5" />
                View Menu
              </Link>
              <Link to="/reservations" className="px-6 py-4 rounded-2xl bg-white/10 border border-white/10 text-white font-semibold">
                Reserve Table
              </Link>
              <a href={waLink} target="_blank" rel="noopener noreferrer" className="px-6 py-4 rounded-2xl bg-emerald-500 text-white font-semibold flex items-center gap-2">
